...
# Compute averages
df_avg = df.groupby(["Group Size", "Array Size"], as_index=False).mean(numeric_only=True)
...
def plot_metric2(metric, title, ylabel, filename):
    ...
    # Add reference lines
    # if metric in ['Comparisons', 'Swaps']:
    #     plt.plot(x_values, 0.6*x_values*np.log2(x_values), 
    #             '--', label='0.6nlog(n)')
        # plt.plot(x_values, x_values*np.log2(x_values), 
        #         ':', label='nlog(n)')
